    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.4fr]">
      ...
      <div className="spabox-card flex flex-col p-5 sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#3f2046]/14 bg-[#feebec] px-4 text-sm font-semibold text-[#3f2046] transition-colors hover:border-[#3f2046]/40"
          ...
          >
            ...
            Filters
            {activeFilterCount > 0 && (
              <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#3f2046] px-1.5 text-[11px] font-bold text-[#feebec]">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        <label className="relative mb-3 block">
          ...
          <SearchIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#9f676d]" />
          <input
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={searchPlaceholder}
            className="h-11 w-full rounded-full border border-[#3f2046]/14 bg-[#feebec] pl-12 pr-4 text-sm font-medium text-[#3f2046] outline-none transition-colors placeholder:text-[#9f676d] focus:border-[#3f2046]/40"
          />
        </label>
        ...
        <p className="mb-2 text-xs font-medium text-[#4c3052]/68">
        ...
        <div className="no-scrollbar max-h-[420px] space-y-2 overflow-y-auto lg:max-h-[560px]">
          {filtered.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#3f2046]/20 p-6 text-center">
              <p className="m-0 text-sm font-medium text-[#4c3052]/72">No treatments match.</p>
              <button
                type="button"
                onClick={() => {
                  clearAll();
                ...
                className="mt-2 text-sm font-semibold text-[#9f676d] underline-offset-4 hover:underline"
              >
                Reset
              </button>
            </div>
                ...
                <button
                  ...
                  className={[
                    ...
                    active
                      ...
                      : "border-[#3f2046]/12 bg-[#feebec]/68 text-[#3f2046] hover:border-[#3f2046]/35",
                  ].join(" ")}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="m-0 truncate text-sm font-semibold">{treatment.name}</p>
                      <p className={`m-0 mt-0.5 text-xs ${active ? "text-[#feebec]/72" : "text-[#4c3052]/68"}`}>
                        {treatment.duration_minutes ? `${treatment.duration_minutes} min · ` : ""}
                        {brand.currency} {treatment.default_price}
                      </p>
                    </div>
                    {active && (
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#feebec] text-xs text-[#3f2046]">
                        ✓
                      </span>
                    )}
                    {treatment.image_url && (
      ...
      <div className="spabox-card p-4 sm:p-6">
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-[#3f2046] px-5 py-3 text-sm font-semibold text-[#feebec] transition-colors hover:bg-[#2e1833] disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting
            ? "Confirming…"
            : selectedSlot
