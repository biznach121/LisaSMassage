      ...
      <div className="spabox-shell">
        ...
        {f.sections.map((section) => (
          <section key={section.title} className="spabox-card p-6 sm:p-7">
            <h2 className="m-0 mb-6 font-serif text-[32px] font-light leading-none text-[#3f2046]">
              {section.title}
            </h2>
            <dl className="space-y-6">
              {section.items.map((item) => (
                <div key={item.q}>
                  <dt className="mb-2 font-semibold text-[#3f2046]">{item.q}</dt>
                  <dd className="m-0 text-[15px] font-medium leading-relaxed text-[#4c3052]/76">
                    {item.a}
                  </dd>
                </div>
              ))}
        ...
        <p className="mt-8 border-t border-[#3f2046]/12 pt-7 text-[15px] font-medium text-[#4c3052]/76">
          {f.contactPrompt}{" "}
          <a href={`mailto:${f.contactEmail}`} className="font-semibold text-[#3f2046] hover:underline">
            {f.contactEmail}
          </a>
        </p>
      </div>
