...
export default function LocationPage() {
  return (
    <main className="bg-[#f3e0e1] text-[#3f2046]">
      <section className="mx-auto grid min-h-[calc(100svh-120px)] max-w-6xl gap-10 px-4 py-12 sm:px-8 sm:py-16 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:py-20">
        <div>
          <p className="m-0 mb-4 text-[12px] font-semibold uppercase text-[#9f676d]">
            Visit us
          </p>
          ...
          <p className="mt-6 max-w-[420px] text-[16px] font-medium leading-[1.4] text-[#4c3052]/80 sm:mt-7 sm:text-[18px]">
            {brand.microTag}. Tap below to open the shop location in Google Maps.
          </p>

          <div className="mt-9 grid gap-3 text-[17px] font-semibold text-[#3f2046]/82">
            <a href={`tel:${brand.contact.phoneTel}`} className="transition-colors hover:text-[#9f676d]">
              Call {brand.contact.phone}
            </a>
            {brand.contact.textPhone ? (
              <a
                href={`sms:${brand.contact.textPhoneTel ?? brand.contact.phoneTel}`}
                className="transition-colors hover:text-[#9f676d]"
              >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-9 inline-flex h-14 items-center justify-center bg-[#3f2046] px-7 text-[13px] font-semibold uppercase text-[#feebec] transition-transform hover:scale-105"
          >
            Open in Google Maps
          </a>
        </div>

        <div className="overflow-hidden rounded-[24px] border border-[#3f2046]/12 bg-[#ebcacc] shadow-[0_26px_80px_rgba(63, 32, 70,0.12)] sm:rounded-[28px]">
          <iframe
