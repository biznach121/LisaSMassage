      ...
      <div className="spabox-shell">
        <header className="spabox-hero mb-8">
          ...
          <p className="spabox-lede">
            ...
            <Link href="/sitemap.xml" className="font-semibold text-[#3f2046] hover:underline">
              /sitemap.xml
            </Link>
            . For LLM agents, see{" "}
            <Link href="/llms.txt" className="font-semibold text-[#3f2046] hover:underline">
              /llms.txt
            </Link>
            .
          </p>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {STATIC_LINKS.map((s) => (
            <Section key={s.title} title={s.title}>
              {s.links.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="transition-colors hover:text-[#3f2046]">
                    {l.label}
                  </Link>
                </li>
              ))}
          ...
          {categories.length > 0 && (
            <Section title="Categories">
              {categories.map((c) => (
                <li key={c.slug}>
                  <Link href={`/categories/${c.slug}`} className="transition-colors hover:text-[#3f2046]">
                    {c.name}
                  </Link>
                </li>
              ))}
          ...
          {collections.length > 0 && (
            <Section title="Collections">
              {collections.map((c) => (
                <li key={c.slug}>
                  <Link href={`/collections/${c.slug}`} className="transition-colors hover:text-[#3f2046]">
                    {c.name}
                  </Link>
                </li>
              ))}
              ...
              {products.map((p) => (
                <li key={p.slug}>
                  <Link
                    ...
                    className="transition-colors hover:text-[#3f2046]"
                  >
                    {p.name}
                  </Link>
                </li>
          ...
          {services.length > 0 && (
            <Section title={`Services (${services.length})`}>
              {services.map((p) => (
                <li key={p.slug}>
                  <Link href="/services" className="transition-colors hover:text-[#3f2046]">
                    {p.name}
                  </Link>
                </li>
              ))}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="spabox-card p-6">
      <p className="mb-4 font-semibold text-[12px] uppercase text-[#9f676d]">
        {title}
      </p>
      <ul className="m-0 list-none space-y-2 p-0 text-sm font-medium text-[#4c3052]/72">
        {children}
      </ul>
    </div>
  );
