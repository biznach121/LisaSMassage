...
export function CartPill() {
  ...
  return (
    <button
      ...
      className="relative grid h-11 w-10 place-items-center text-[#3f2046] transition-transform hover:scale-105"
    >
      <svg
        viewBox="0 0 24 24"
        fill="none"
      ...
      <span className="absolute right-0 top-0 grid h-4 min-w-4 place-items-center rounded-full border border-[#3f2046]/55 bg-[#feebec] px-1 text-[10px] font-bold leading-none text-[#3f2046]">
        {count}
      </span>
    </button>
  );
...
export function CartPillSkeleton() {
  return (
    <span
      aria-hidden
      className="inline-flex h-11 w-10 items-center justify-center text-[#3f2046]"
    />
  );
}
