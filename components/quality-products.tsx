...
export function QualityProducts({
  ...
  return (
    <section ref={sectionRef} className="relative isolate overflow-hidden bg-[#f3e0e1] px-4 pb-20 pt-12 text-[#3f2046] sm:px-8 lg:pb-28">
      <QualityVines />
      <div className="relative z-10 mx-auto max-w-[1180px]">
        ...
        <div className="mt-10 flex items-center justify-center gap-4">
          ...
          />
          <span className="text-[13px] font-semibold text-[#3f2046]/70">
...
function ProductCard({
  ...
  return (
    <article
      ...
      className={[
        "relative min-h-[560px] cursor-pointer overflow-hidden rounded-[180px] border border-white/25 px-7 pb-8 pt-[82px] shadow-[0_24px_70px_rgba(63, 32, 70,0.08)] transition-transform hover:-translate-y-1 sm:h-[660px] sm:rounded-[240px] sm:px-12 sm:pt-[104px] lg:h-[720px] lg:px-14 lg:pt-[112px]",
        position === 0 ? "lg:translate-y-[-16px]" : "hidden lg:block lg:translate-y-[24px]",
        isOlive ? "bg-[#9f676d] text-white" : "bg-[#ebcacc] text-[#3f2046]",
      ].join(" ")}
    >
      <div
        className={[
          "pointer-events-none absolute inset-3 rounded-[inherit] border",
          isOlive ? "border-white/16" : "border-[#3f2046]/10",
        ].join(" ")}
      />
      <div className="relative z-10 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4">
        ...
        <button
          ...
          className={[
            ...
            isOlive ? "bg-[#f9eaeb] text-[#8b565a]" : "bg-[#3f2046] text-white",
          ].join(" ")}
        >
      ...
      <div
        className={[
          "absolute inset-x-0 bottom-7 mx-auto h-[235px] w-[84%] overflow-hidden rounded-[999px] shadow-[0_18px_44px_rgba(63, 32, 70,0.18)] sm:bottom-9 sm:h-[300px] sm:w-[80%] lg:h-[330px]",
          isOlive ? "bg-[#8b565a]" : "bg-[#e0b0b2]",
        ].join(" ")}
      >
        {image ? (
          <Image
...
function CarouselButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="grid h-12 w-12 place-items-center rounded-full border border-[#3f2046]/35 text-[#3f2046] transition-colors hover:bg-[#3f2046] hover:text-[#f3e0e1] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-[#3f2046]"
    >
      <svg
        viewBox="0 0 24 24"
        fill="none"
