...
export function ServiceBands() {
  return (
    <section className="h-svh bg-[#f3e0e1] px-1 pb-1 text-white">
      <div className="mx-auto grid h-full max-w-[1680px] grid-rows-3 overflow-hidden">
        {brand.serviceBands.map((service) => (
          <article
            key={service.title}
            className="group relative min-h-0 overflow-hidden border-t-[18px] border-[#f3e0e1] first:border-t-0"
          >
            <Image
              src={service.image}
              ...
              unoptimized
            />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(44, 14, 46,0.52),rgba(44, 14, 46,0.18)_36%,rgba(44, 14, 46,0.02)_72%)]" />
            <div className="relative z-10 flex h-full flex-col justify-center px-9 py-7 sm:px-16 lg:px-[11%]">
              ...
              >
                <ServiceGlyph />
                <span className="inline-flex h-14 items-center bg-[#f9eaeb]/92 px-7 text-[13px] font-semibold uppercase transition-colors group-hover:bg-white sm:h-16 sm:px-9">
                  {service.ctaLabel}
                </span>
              </Link>
            </div>
